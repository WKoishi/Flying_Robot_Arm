...
extern "C" void servo_ctrl_task(void const * argument)
{
    ...
    for (;;)
    {
        ...
        get_time_period(&servo_time);
        if (ret_val)
            usb_printf("Y1 %d,%d,%f,%f,%d,%f\r\n", servo_1.position, servo_1.velocity, 
                servo_1.load_ratio, servo_1.voltage, servo_1.temperature, servo_1.current);
        else
            usb_printf("N1 %d,%d,%f,%f,%d,%f\r\n", servo_1.position, servo_1.velocity, 
                servo_1.load_ratio, servo_1.voltage, servo_1.temperature, servo_1.current);
        ...
        get_time_period(&servo_time);
        if (ret_val)
            usb_printf("Y2 %d,%d,%f,%f,%d,%f\r\n", servo_2.position, servo_2.velocity, 
                servo_2.load_ratio, servo_2.voltage, servo_2.temperature, servo_2.current);
        else
            usb_printf("N2 %d,%d,%f,%f,%d,%f\r\n", servo_2.position, servo_2.velocity, 
                servo_2.load_ratio, servo_2.voltage, servo_2.temperature, servo_2.current);
