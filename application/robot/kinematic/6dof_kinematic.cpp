...
inline float arm_cos(float x)
{
    return arm_cos_f32(x);
}

inline float arm_sin(float x)
{
...
static void EulerAngleToRotMat(const float* _eulerAngles, float* _rotationM)
{
    ...
    cc = arm_cos(_eulerAngles[0]);
    cb = arm_cos(_eulerAngles[1]);
    ca = arm_cos(_eulerAngles[2]);
    sc = arm_sin(_eulerAngles[0]);
    sb = arm_sin(_eulerAngles[1]);
    sa = arm_sin(_eulerAngles[2]);
...
DOF6Kinematic::SolveFK(const DOF6Kinematic::Joint6D_t &_inputJoint6D, DOF6Kinematic::Pose6D_t &_outputPose6D)
{
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        cosq = arm_cos(q[i]);
        sinq = arm_sin(q[i]);
        cosa = arm_cos(DH_matrix[i][3]);
        sina = arm_sin(DH_matrix[i][3]);
                            ...
                            DOF6Kinematic::IKSolves_t &_outputSolves)
{
    ...
    {
        cosqs = arm_cos(qs[ind_arm] + DH_matrix[0][0]);
        sinqs = arm_sin(qs[ind_arm] + DH_matrix[0][0]);
        ...
        {
            cosqa[0] = arm_cos(qa[ind_elbow][0] + DH_matrix[1][0]);
            sinqa[0] = arm_sin(qa[ind_elbow][0] + DH_matrix[1][0]);
            cosqa[1] = arm_cos(qa[ind_elbow][1] + DH_matrix[2][0]);
            sinqa[1] = arm_sin(qa[ind_elbow][1] + DH_matrix[2][0]);
            ...
            if (1.0f == cosqw || -1.0f == cosqw)
            {
                if (0 == ind_arm)
                {
                    ...
                    cosqw = arm_cos(_lastJoint6D.a[3] + DH_matrix[3][0]);
                    sinqw = arm_sin(_lastJoint6D.a[3] + DH_matrix[3][0]);
                    qw[0][2] = atan2f(cosqw * R36[3] - sinqw * R36[0], cosqw * R36[0] + sinqw * R36[3]);
                    qw[1][2] = _lastJoint6D.a[5];
                    cosqw = arm_cos(_lastJoint6D.a[5] + DH_matrix[5][0]);
                    sinqw = arm_sin(_lastJoint6D.a[5] + DH_matrix[5][0]);
